...
(function (ns) {
	...
	_b.showTip = function () {
		...
		$('<div style="color:red;width:100%" class="gwg_balloon_tip">Tip: Didn\'t want this definition pop-up? Try setting a trigger key in <a target="_blank" href="' + option_page + '">Extension Options</a>.</div>').insertAfter('#gwg_balloon_define');	
	};
	...
	_b.render = function (resp, status, xhr) {
		...
		if ( validateWord(this.word) && !this.options.dblclick_key && !this.options.select_key ) {
			_b.showTip();
		}
	...
	_b.error = function (xhr, status, error) {
		...
		var html = xhr.status == 404 ? '<div><div style="color: #666">No results were found.</div><div style="padding: 10px 0 0 0;color:#666;text-align:right"><a style="font-weight:normal" target="_blank" href="https://www.goodwordguide.com/google.html#gsc.tab=0&gsc.q=' + term + '&gsc.sort=">Search the web for "<strong style="color:inherit;font-weight:bold">' + $.trim(this.selection) + '</strong>" <img style="display:inline" src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAYAAABWdVznAAAACXBIWXMAAA7DAAAOwwHHb6hkAAAAXklEQVR4nK2QwQ3AMAwCs1N28k7eiZ3oI7IcU6efBomXOREyxhUZ2brTdNAcVB2BaJgCVcDAalJLXsB+iLAjm1pAwzHWHD3gWMcMg/ERMjKfFOHVqMEGqEM/gKP/6gE2f+h+3kvjIwAAAABJRU5ErkJggg=="></a></div><iframe style="width:1px;height:1px;display:none" src="' + ga + '?error=' + term + '&referer=' + window.location.href + '&app_name=Chrome EXT: ' + headers['GWG_VERSION'] + '" frameborder="0" width="1" height="1" scrolling="no"></iframe></div>' : 'Internet connection timed out! Please try again.';
